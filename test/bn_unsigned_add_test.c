...
int TEST_bn_unsigned_add()
{
    ...
    for (case_idx = 0; case_idx < case_num; case_idx++) {
        ...
        if (!TEST_true(bn_dec2bn(&a, a_str) != 0, "a赋值失败") ||
            !TEST_true(bn_dec2bn(&b, b_str) != 0, "b赋值失败") ||
            !TEST_true(bn_dec2bn(&sum, expected_str) != 0, "sum赋值失败"))
        {
            printf("失败（初始化）\n");
            case_ok = 0;
            test_result = 0;
        ...
        // 基础功能测试
        if (!TEST_true(bn_unsigned_add(ret, a, b), "bn_unsigned_add基础调用失败") ||
            !TEST_BN_equal(sum, ret, "基础加法A+uB"))
        {
            printf("失败（基础功能）\n");
            case_ok = 0;
            test_result = 0;
